...
setwd("/Users/muralikorrapati/DSLA1/citibike/citibike")
getwd()
rawdf<- read.csv(file = "raw/201707-citibike-tripdata.csv",header=TRUE, stringsAsFactors=FALSE)
...
medianage_df = df %>% 
  select(age,startname, start.station.longitude, start.station.latitude) %>% 
  dplyr::filter(start.station.latitude > manhattan_bounds$south & start.station.latitude < manhattan_bounds$north & 
                  start.station.longitude < manhattan_bounds$east & start.station.longitude > manhattan_bounds$west) %>%
...
medianage_df = df %>% 
  select(age,endname, end.station.longitude, end.station.latitude) %>% 
  dplyr::filter(end.station.latitude > manhattan_bounds$south & end.station.latitude < manhattan_bounds$north & 
                  end.station.longitude < manhattan_bounds$east & end.station.longitude > manhattan_bounds$west) %>%
  group_by(endname, end.station.longitude, end.station.latitude) %>%
  dplyr::summarize(median = median(age)) %>%
  top_n(20)
# medianage_df = medianage_df[sample(nrow(medianage_df),replace=F,size=0.5*nrow(medianage_df)),]
...
medianage_df = df %>% 
  select(age, startid, start.station.longitude, start.station.latitude) %>% 
  dplyr::filter(start.station.latitude > manhattan_bounds$south & start.station.latitude < manhattan_bounds$north & 
                  start.station.longitude < manhattan_bounds$east & start.station.longitude > manhattan_bounds$west) %>%
  group_by (startid, start.station.longitude, start.station.latitude) %>%
  summarize(median = median(age)) %>%
  ungroup()
medianage_df

ggmap(get_map('New York',zoom=13, maptype='terrain')) +
...
df = df %>% select(duration, tripduration.min, tripduration.mingroup,
                   ...
                   startname, start.station.latitude, start.station.longitude,
                   stopday, stoptime, stophour, endid,
                   endname, end.station.latitude, end.station.longitude)
...
bike_icon = makeIcon('www/bike_icon.png', iconWidth = 25, iconHeight = 38)
leaflet(data = map_df) %>% 
  addTiles() %>% 
